...
const initColumns = () => {
  return [
    ...
    },
    {
      ...
      key: "actions",
      render(row) {
        return h(Actions, { upData: () => initMenu("修改子菜单", row), delData: () => delMenu(row.id) })
      }
    }
...
const initMenu = (title,row) => { 
  const m = window.$modal.create({
   ...
   content: () => h(UpModal, {close:()=>closeModal(m),row,getlist,total:total.value,parent_id:query.id,prefix:query.prefix})
  })
}
...
const getlist = async () => { 
  loading.value = true
  const res = await proxy.$api.menu.getMenuList({ ...queryFormData.value, ...page,parent_id: parseInt(query.id) })
  if (res.code === 200) { 
    ...
    <XwyaForm  label-placement="left" :item-list="queryFormItem" v-model="queryFormData" :row="5">
      ...
      <template #button>
        <div class="flex gap-2">
            <XwyaPopover text="新增">
          <XwyaButton  v-has="'xwya:menu:add'" @click="initMenu('新增子菜单')" circle type="info" iconSize="20" icon="iconify gg--math-plus"  />
        </XwyaPopover>
