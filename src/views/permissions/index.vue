...
const initColumns = () => {
  return [
...

    {
      ...
      key: "actions",
      render(row) {
        return h(Actions, { upData: () => initPermissions("修改菜单", row), delData: () => delPermissions(row.permission_id), toSub: () => push(`/user/buttonPermissions?id=${row.permission_id}&prefix=${row.permission_name}`) })
      }
    }
...
const getlist = async () => { 
  loading.value = true
  const res = await proxy.$api.permissions.getPermissionsList({ ...queryFormData.value, ...page })
  if (res.code === 200) { 
